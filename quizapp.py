...
class Quiz:
    ...
    def display_question(self):
        q_no = Label(root,text=question[self.q_no], width=60,font=("arial",16,"bold"),anchor="w")
    ...
    def display_result(self):
        ...
        correct = f" Your Correct answers are: {self.correct}"
        wrong = f"Your Wrong answers are : {wrong_count}"
        score = int(self.correct/self.data_size * 100)
        result = f"You Scored :  {score}%"
        mb.showinfo("Your Results Acquired",f"{result}\n{correct}\n{wrong}")

    def buttons(self):
        next_button = Button(root,text="Next", bg="green",command=self.next_btn,fg="white",font=("aerial",16,"bold"))
